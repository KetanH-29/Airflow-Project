...
def incremental_append():
    ...
    base_path = "s3a://ketan-mirror-bucket/MySQL_DB/test_db/tables/previous/current"

    # Iterate through each table
    for table in tables:
        print(f"Processing table: {table}")
        source_path = f"{base_path}/full_load/tables/{table}"
        incremental_output_path = f"{base_path}/incremental/tables/{table}"
        data_exists = False
        attempts = 0
